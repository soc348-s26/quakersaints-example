...
if (file.exists(here("data/qs_tokens.rda"))) {
  ...
  message("qs_tokens file does not exist")
}
...
qs_tokens_stories |> group_by(section_title, word_stemmed) |> 
  summarize(n = n()) |>
cast_dtm( term = word_stemmed, document=section_title, n) -> quakersaints_dtm

save(quakersaints_dtm, file = here("data/quakersaints_dtm.rda"))
```
...
if (!file.exists(here("data/fit.rda"))) {
  K <- c(3:20)
...
```{r}
save(story_level_data, file = here("data/story_level_data.rda"))
```
```{r}
table(story_level_data$`Younger Readers`, story_level_data$any_quaker_words
      )
```
```{r}
story_level_data |>
  select(section_title, n_tokens, `George Fox`,
         `Quaker Words`, `Younger Readers`, cluster,
         topic) |>
    write.csv(file = here("poster/story_level_data.csv"))
```
